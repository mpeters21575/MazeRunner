...
{
    public IReadOnlyCollection<string> Names => new[] { "register" };
    public string Usage => "register <name>";
    public async Task<bool> TryExecuteAsync(string[] parts, CancellationToken ct)
    {
        try
        {
            var name = parts.Length > 1 ? string.Join(" ", parts.Skip(1)) :
                AnsiConsole.Prompt(new TextPrompt<string>("name: ").Validate(v => string.IsNullOrWhiteSpace(v) ? ValidationResult.Error("required") : ValidationResult.Success()));
